...
Queue.prototype.getByIndex =
    Queue.prototype.getJobsByIndex = function(index,indexValue,fn) {
        return Job.getByIndex(this,index,indexValue,fn);
    };

Queue.prototype.getById =
    Queue.prototype.getJobById = function(id,fn) {
        return Job.getById(this,id,fn);
    };
...
Queue.prototype.setting = function (name, fn) {
    this.client.hget('q:' + this.name + ':settings', name, fn);
    return this;
};
...
Queue.prototype.process = function (type, n, fn) {
    ...
    while (n--) {
        ...
        worker.on('job complete', function (job) {
            self.client.incrby('q:' + self.name + ':stats:work-time', job.duration);
        });
...
Queue.prototype.types = function (fn) {
    this.client.smembers('q:' + this.name + ':job:types', fn);
    return this;
};
...
Queue.prototype.state = function (state, fn) {
    this.client.zrange('q:' + this.name + ':jobs:' + state, 0, -1, fn);
    return this;
};
...
Queue.prototype.workTime = function (fn) {
    this.client.get('q:' + this.name + ':stats:work-time', function (err, n) {
...
Queue.prototype.card = function (state, fn) {
    this.client.zcard('q:' + this.name + ':jobs:' + state, fn);
    return this;
};

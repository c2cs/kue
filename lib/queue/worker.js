 ...
 * @api private
 */
Worker.prototype.next = function(fn){
    var self = this;
    if (!self.running) return;
    self.getJob(function onGetJob(err, job) {
...
Worker.prototype.getJob = function (fn) {
    ...
    client.blpop(prefix + ':' + self.type + ':jobs', 0, function onBlpop(err) {
        if (err) return fn(err);		// SAE: Added to avoid crashing redis on zpop
        if (!self.running) {
            return client.lpush(prefix + ':' + self.type + ':jobs', [1], function () {
                fn(err /*||"shutdown pushback"*/);
            });
        }
        // Set job to a temp value so shutdown() knows to wait
        self.job = true;
        self.zpop(prefix + ':jobs:' + self.type + ':inactive', function onZpop(err, id) {
            if (err || !id) {
                self.job = false;
...
Worker.prototype.shutdown = function (fn, timeout) {
    ...
    var _fn = function (job) {
        ...
        self.client.lpush(prefix + ':' + self.type + ':jobs', [1], function () {});
